...
Spring Batch에서 `LIMIT/OFFSET` 페이징과 Keyset 페이징의 특성을 로컬 PostgreSQL로 비교하기 위한 재현 프로젝트다. 현재는 **1단계 기반 구성만 완료**되어 있으며 Reader 구현과 성능 측정 결과는 아직 없다.
...
OFFSET/Keyset Reader, checksum writer, 재시작 정합성, 인덱스 전환, EXPLAIN, GC/Old Gen 측정, 전체 36회 실험과 블로그 초안은 다음 단계 작업이다. 현재 출력은 Reader 성능 수치로 사용할 수 없다.
